...
Публичные страницы:
- `/` – лендинг с секциями и формой “Мы вам перезвоним”.
- `/rooms` – каталог номеров с фильтрами.
- `/rooms/[slug]` – подробная страница номера с формой заявки.
- `/prices` – базовые и сезонные цены (со ссылками на номера).

Админка:
- `/admin` – индекс с быстрыми ссылками.
- `/admin/rooms` – управление номерами.
- `/admin/prices` – управление сезонными ценами.
- `/admin/requests` – обработка заявок.

Навигация в шапке и футере ведёт на `/rooms`, `/prices` и `/` (якоря используются только на главной странице).

### Пользовательский сценарий (гость)
1. Откройте главную страницу `/`.
2. Перейдите в каталог `/rooms`, выберите номер и откройте его карточку.
3. На странице `/rooms/[slug]` отправьте заявку на бронирование.
4. При необходимости посмотрите сезонные цены на `/prices` и вернитесь в каталог.

### Пользовательский сценарий (администратор)
1. Откройте `/admin` и перейдите в “Сезонные цены”.
2. Добавьте сезонный тариф для нужного номера.
3. Откройте `/prices` и проверьте отображение сезонных правил и ссылок на номера.
